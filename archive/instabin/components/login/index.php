...
if(isset($_SESSION['username'])) 
    {
		...
		if (isset($_POST['ta'])) {
			...
			if (file_exists("$user/$string/index.html")) {
				echo "<script>alert('The custom URL $string already exists')</script>";
			}

			else {
				mkdir("$user/$string", 0700);
				$myFile = "$user/$string/index.html";
				$fh = fopen($myFile, 'w') or die("Can't open file");
				$stringData = $_POST['ta'];
				fwrite($fh, $stringData);
				fclose($fh);
	
				header("Location: $user/$string");
			}
		}
	}
	else {
...
shortcut = {
    ...
    <body id="body" onload="update()"> 
        ...
        <form name="code" method="post">
			...
			<?php
			if (isset($_GET['url'])) {
				echo "<script>document.getElementById('code').value = ''; window.localStorage.setItem('value', area.value); window.localStorage.setItem('timestamp', (new Date()).getTime()); updateLog(true);</script><textarea id='code' onkeyup='update()' onclick='Set_Count_Mouse()' height='100%' width='100%' autofocus required name='ta' wrap='hard' spellcheck='false' class='code' placeholder='Type your HTML here...'>";
				echo file_get_contents($_GET['url']);
				echo "</textarea>";
			}
			else {
				echo "<textarea id='code' onkeyup='update()' onclick='Set_Count_Mouse()' height='100%' width='100%' autofocus required name='ta' wrap='hard' spellcheck='false' class='code' placeholder='Type your HTML here...'></textarea>";
            }
			?>
			</div>
			
			<div id="footer">
				<?php echo "Logged in as <b><a href='$user'>$user</a></b>";?>
				<a style="margin-left: 10px;" href="index.php"><a href="home.php">Home</a> | <a href="accountPage.php">Account</a> | <a href="javascript: help()">Hotkeys</a> | <a href="logout.php">Logout</a>
				<input placeholder="Custom URL..." id="custom" style="font-size: 10px; margin-left: 10px;" type="text" name="custom"/>
				<input type="button" value="Save" onclick="document.code.submit();"/>
				<input type="button" value="Save To Account" onclick="document.code2.submit();"/>
			</div>
        </form>
		<form style="display: none;" id="accountCode" name="code2" method="post">
<?php
		...
		echo "<form method='post' id='accountCode' name='code2'><script>document.getElementById('code').value = ''; window.localStorage.setItem('value', area.value); window.localStorage.setItem('timestamp', (new Date()).getTime()); updateLog(true);</script><textarea id='code' onkeyup='update()' onclick='Set_Count_Mouse()' height='100%' width='100%' autofocus required name='accountTa' wrap='hard' spellcheck='false' class='code' placeholder='Type your HTML here...'>$data</textarea></form>";
		if (isset($_POST['accountTa'])) {
			$sql = "UPDATE members SET data = '$_POST[accountTa]'
			WHERE username = '$user'";
